...
int Data::getValue(int x, int y) { return this->table[x][y]; }
	...
	for (int i = 0; i < x; i++) {
		...
		if (choice != '*') {
			for (int j = 0; j < y; j++) {
				if (choice == '+') {
					this->data->table[i][j] =
						oldTable[i][j] + other.data->table[i][j];
				}
				if (choice == '-') {
					this->data->table[i][j] =
						oldTable[i][j] - other.data->table[i][j];
...
int Matrix::getRefCount() { return this->data->refCount; }

int Matrix::getX() const { return this->data->x; }

int Matrix::getY() const { return this->data->y; }
...
Matrix::~Matrix() { this->resetValue(false); }
